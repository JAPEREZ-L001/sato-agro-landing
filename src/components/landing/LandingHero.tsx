    ...
    <section className="landing-section bg-[var(--sato-color-bg-warm)] overflow-hidden" id="ejemplo" aria-label="Hero principal">
      <div className="landing-container">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-8">
          ...
          {/* Text Content */}
          <LandingReveal className="flex-1 text-center lg:text-left flex flex-col items-center lg:items-start max-w-3xl mx-auto lg:mx-0">
            <p className="text-sm font-bold text-[var(--sato-color-primary)] tracking-wide uppercase mb-4">
              {eyebrow}
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
              {isExternalUrl(primaryCtaLink) ? (
                <a href={primaryCtaLink} className="w-full sm:w-auto">
                  <Button size="lg" className="w-full sm:w-auto bg-[var(--sato-color-primary)] hover:bg-[var(--sato-color-primary-active)] text-white border-none shadow-sm rounded-[var(--sato-radius-button)] text-base h-12 px-8">
                    {primaryCtaText}
                  </Button>
                </a>
              ) : (
          ...
          {/* Media / Mock UI */}
          <LandingReveal delay={150} className="flex-1 w-full max-w-md lg:max-w-none relative">
            <div className="relative mx-auto w-full max-w-[340px] aspect-[4/5] bg-white rounded-[24px] shadow-2xl border border-[var(--sato-color-border)] overflow-hidden flex flex-col">
              {/* Mock App Header */}
                  ...
                  <>
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        <div key={i} className="bg-white p-3 rounded-lg border border-[var(--sato-color-border)] shadow-sm flex items-center justify-between">
            ...
            {/* Decorative elements */}
            <div className="absolute -top-6 -right-6 w-24 h-24 bg-amber-400/20 rounded-full blur-2xl -z-10" aria-hidden="true"></div>
            <div className="absolute -bottom-10 -left-10 w-32 h-32 bg-[var(--sato-color-primary)]/10 rounded-full blur-2xl -z-10" aria-hidden="true"></div>
          </LandingReveal>
